...
function setupInteractions() {
  ...
  cleanupKeyboard = setupKeyboardHandler("irc-files", {
    ArrowDown: handleNavigateDown,
    ArrowUp: handleNavigateUp,
    Enter: handleToggle,
  });

  cleanupClick = setupClickHandler("irc-files", ".fb-row", (e, target) => {
...
export function startIRCFiles() {
  filesWindow.start();
  setTimeout(setupInteractions, 100);
}

export function stopIRCFiles() {
  filesWindow.stop();
  if (cleanupKeyboard) cleanupKeyboard();
  if (cleanupClick) cleanupClick();
}
