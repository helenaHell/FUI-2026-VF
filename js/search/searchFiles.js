...
function scrollToSelected() {
  const body = document.querySelector("#search-files .fb-body");
  const selectedRow = document.querySelector("#search-files .fb-row.selected");
...
function setupInteractions() {
  ...
  cleanupKeyboard = setupKeyboardHandler("search-files", {
    ArrowDown: handleNavigateDown,
    ArrowUp: handleNavigateUp,
    Enter: handleToggle,
  });

  cleanupClick = setupClickHandler("search-files", ".fb-row", (e, target) => {
...
export function startSearchFiles() {
  filesWindow.start();
  setTimeout(setupInteractions, 100);
}

export function stopSearchFiles() {
  filesWindow.stop();
  if (cleanupKeyboard) cleanupKeyboard();
  if (cleanupClick) cleanupClick();
}
